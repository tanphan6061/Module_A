...
@section('content')
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
        ...
        <form method="post" class="needs-validation" novalidate action="{{route('sessions.store',[$event])}}">
            <div class="row">
                <div class="col-12 col-lg-4 mb-3">
                    <label for="selectType">Type</label>
                    <select class="form-control {{$errors->has('type')?'is-invalid':''}}" id="selectType" name="type">
                        <option value="talk" {{old('type')=='talk'?'selected':''}}>Talk
                        </option>
                ...
                <div class="col-12 col-lg-4 mb-3">
                    ...
                    <select class="form-control {{$errors->has('room_id')?'is-invalid':''}}" id="selectRoom"
                        ...
                        @foreach($event->rooms as $room)
                            <option
                                value="{{$room->id}}" {{old('room_id')==$room->id ?'selected':''}}>{{$room->name}}
